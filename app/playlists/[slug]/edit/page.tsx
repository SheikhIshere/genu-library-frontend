...
export default function EditPlaylistPage() {
  ...
  useEffect(() => {
    if (!slug) return;
      ...
      .then((data) => {
        ...
        const vis = d.is_public !== false ? 'public' : 'private';

        setTitle(t);
        setDescription(desc);
        setVisibility(vis);
        setInitialState({ title: t, description: desc, visibility: vis });
        setIsLoading(false);
      })
      .catch(() => {
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        await books.playlistUpdate(slug, {
          title: title.trim(),
    ...
    [title, description, visibility, slug, router]
  );
  ...
  const visDescriptions: Record<string, string> = {
    public: 'Discoverable in the catalog, search results, and guild recommendations.',
    private: 'Restricted exclusively to your personal archive.',
        ...
        <div className="max-w-7xl mx-auto flex flex-col gap-4">
          ...
          <div className="space-y-2.5 max-w-3xl">
            ...
            <p className="font-body text-base text-text-secondary max-w-2xl leading-relaxed">
              Modify the settings for <span className="text-text-primary italic font-display">{title}</span>.
            </p>
          </div>
        </div>
      </div>
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <section className="p-4 md:p-6 rounded-xl bg-surface border border-border space-y-4 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <Link className="font-body text-body-small text-text-muted hover:text-text-primary transition-colors text-center sm:text-left py-2" href={`/playlists/${slug}`}>
                Discard &amp; Cancel
              </Link>
              <div className="flex flex-col-reverse sm:flex-row items-center gap-3">
                <button
                  className="w-full sm:w-auto px-5 py-2.5 rounded-lg border border-border text-text-primary hover:border-primary hover:text-primary font-body text-body-small font-medium transition-all flex items-center justify-center gap-2 active:scale-95"
                  type="button"
                  onClick={() => router.push(`/playlists/${slug}`)}
                >
            ...
            <div className="pt-1 border-t border-border flex items-center gap-2">
              ...
              <p className="font-body text-xs text-text-muted">
                Changes are saved immediately and applied across the Sovereign Library Index.
              </p>
            </div>
          </section>
        </form>
